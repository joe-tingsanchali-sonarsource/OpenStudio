...
namespace model {
  ...
  namespace detail {
    ...
    CoilHeatingElectric_Impl::CoilHeatingElectric_Impl(const openstudio::detail::WorkspaceObject_Impl& other, Model_Impl* model, bool keepHandle)
      : StraightComponent_Impl(other, model, keepHandle) {
      OS_ASSERT(other.iddObject().type() == CoilHeatingElectric::iddObjectType());
    }

    CoilHeatingElectric_Impl::CoilHeatingElectric_Impl(const CoilHeatingElectric_Impl& other, Model_Impl* model, bool keepHandle)
      : StraightComponent_Impl(other, model, keepHandle) {}

    const std::vector<std::string>& CoilHeatingElectric_Impl::outputVariableNames() const {
      static const std::vector<std::string> result{"Heating Coil Air Heating Energy", "Heating Coil Air Heating Rate",
    ...
    Schedule CoilHeatingElectric_Impl::availabilitySchedule() const {
      boost::optional<Schedule> value =
        getObject<ModelObject>().getModelObjectTarget<Schedule>(OS_Coil_Heating_ElectricFields::AvailabilityScheduleName);
      if (!value) {
    ...
    bool CoilHeatingElectric_Impl::setAvailabilitySchedule(Schedule& schedule) {
      bool result = setSchedule(OS_Coil_Heating_ElectricFields::AvailabilityScheduleName, "CoilHeatingElectric", "Availability", schedule);
      return result;
    }
